                ...
                for (final MapleCharacter mc : characters) {
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8810122:
                            case 8820001:
                                mc.getClient().getSession().writeAndFlush(EffectPacket.showOwnBuffEffect(buffid, 13, mc.getLevel(), 1)); // HT nine spirit
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
       ...
       /* this isn't automatic in gms anymore, it's actually done by npc. :)
            ...
            try {
                for (MapleCharacter c : characters) {
                    c.getClient().getSession().writeAndFlush(CField.showEffect("aswan/clear"));
                    switch (mapid) {
                        case 955000100:
                ...
                for (MapleCharacter c : characters) {
                    if (getAllMonstersThreadsafe().size() > 0) {
                        c.dropMessage(-1, "There are " + getAllMonstersThreadsafe().size() + " monsters left.");
                         ...
                         c.getClient().getSession().writeAndFlush(CField.showEffect("quest/party/clear")); // map
                         c.getMap().broadcastMessage(CField.playSound("Party1/Clear")); // client
                         c.getClient().getSession().writeAndFlush(CField.playSound("Party1/Clear")); // map
                    }
                }
            } finally {
                charactersLock.readLock().unlock();
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                    c.getSession().writeAndFlush(MobPacket.spawnAswanMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid)); // TODO effect
                } else {
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                    c.getSession().writeAndFlush(MobPacket.spawnAswanMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0));
                } else {
    ...
    public final int spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos) {
        try {
            ...
            spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
                ...
                public final void sendPackets(MapleClient c) {
                    if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                        c.getSession().writeAndFlush(MobPacket.spawnAswanMonster(monster, effect, 0));
                    } else {
    ...
    public final void spawnFakeMonster(final MapleMonster monster) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                    c.getSession().writeAndFlush(MobPacket.spawnAswanMonster(monster, -4, 0));
                } else {
        ...
        spawnAndAddRangedMapObject(familiar, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (familiar != null && c.getPlayer() != null) {
                    c.getSession().writeAndFlush(CField.spawnFamiliar(familiar, true));
                }
            }
        });
    }
    ...
    public final void spawnMesoDrop(final int meso, final Point position, final MapleMapObject dropper, final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            @Override
            public void sendPackets(MapleClient c) {
                c.getSession().writeAndFlush(CField.dropItemFromMapObject(mdrop, dropper.getTruePosition(), droppos, (byte) 1));
            }
        });
        if (!everlast) {
    ...
    public final void spawnMobMesoDrop(final int meso, final Point position, final MapleMapObject dropper, final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            @Override
            public void sendPackets(MapleClient c) {
                c.getSession().writeAndFlush(CField.dropItemFromMapObject(mdrop, dropper.getTruePosition(), position, (byte) 1));
            }
        });
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (c != null && c.getPlayer() != null && (questid <= 0 || c.getPlayer().getQuestStatus(questid) == 1) && (idrop.getItemId() / 10000 != 238 || c.getPlayer().getMonsterBook().getLevelByCard(idrop.getItemId()) >= 2) && mob != null && dropPos != null) {
                    c.getSession().writeAndFlush(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 1));
                }
            }
        });
//	broadcastMessage(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final Item item, Point pos, final boolean ffaDrop, final boolean playerDrop) {
        ...
        spawnAndAddRangedMapObject(drop, new DelayedPacketCreation() {
            ...
            @Override
            public void sendPackets(MapleClient c) {
                c.getSession().writeAndFlush(CField.dropItemFromMapObject(drop, dropper.getTruePosition(), droppos, (byte) 1));
            }
        });
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (!chr.isHidden()) {
            ...
            chr.getClient().getSession().writeAndFlush(CField.spawnFlags(nodes.getFlags()));
            if (GameConstants.isTeamMap(mapid) && !chr.inPVP()) {
        ...
        for (final MaplePet pet : chr.getPets()) {
            if (pet.getSummoned()) {
                if (chr.isGM()) {
                    chr.getClient().getSession().writeAndFlush(PetPacket.showPet(chr, pet, false, false));
                } else {
        ...
        if (!chr.isInBlockedMap() && chr.getLevel() > 10) {
            chr.getClient().getSession().writeAndFlush(CField.getPublicNPCInfo());
        }
        ...
        if (chr.getBuffedValue(MapleBuffStat.MONSTER_RIDING) != null && !GameConstants.isResist(chr.getJob())) {
            ...
            if (mapid == 914000000 || mapid == 927000000) {
                ...
                chr.getClient().getSession().writeAndFlush(CWvsContext.temporaryStats_Balrog(chr));
                ...
                chr.getClient().getSession().writeAndFlush(CField.musicChange(MapConstants.FM_BGM));
    ...
    public MapleMonster spawnPvpTarget(MapleCharacter chr) {
        ...
        chr.getClient().getSession().writeAndFlush(MobPacket.killMonster(target.getObjectId(), (byte) 1));
