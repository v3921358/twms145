...
function action(mode, type, selection) {
    ...
    if (cm.getInfoQuest(23007).indexOf("vel00=1") == -1 && cm.getInfoQuest(23007).indexOf("vel01=1") == -1) {
        if (status == 0) {
            ...
            cm.sendNext("怎麼會來這裡？這裡是禁止出入的地方。");
        } else if (status == 2) {
            cm.sendNextPrevS("你是誰？", 2);
        } else if (status == 3) {
            cm.sendNextPrev("我…在這裡，往上看。");
        } else if (status == 4) {
        ...
        if (status == 0) {
            cm.sendNext("我是…#r傑利麥勒博士#k的實驗者。我叫做#b裴勒#k…雖然不知道你們怎麼跑進來的，快點出去！要是被博士發現的話，就完蛋了！");
        } else if (status == 1) {
            cm.sendNextPrevS("實驗者？傑利麥勒？到底在說什麼啊？這裡是什麼地方？你為什麼要進去裡面啊？", 2);
        } else if (status == 2) {
            cm.sendNextPrev("你不知道傑利麥勒？　傑利麥勒博士…… 黑色翅膀的瘋狂科學家！ 這裡是傑利麥勒的研究室，傑利麥勒在這裡進行人體實驗…");
        } else if (status == 3) {
