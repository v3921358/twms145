...
var status = 0;
var minLevel = 50; // according to Nexon it's 30, but it's actually a 50 requirement.
var maxLevel = 255;
var minPartySize = 3;
var maxPartySize = 6;

var brokenGlassesCount = 13; // code custom quest data is on the todo list
...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (selection == 0) {
			if (cm.getParty() == null) {
				cm.sendYesNo("You can participate in the party quest by creating a party. Would you like to use the Party Search function to find party members?");
